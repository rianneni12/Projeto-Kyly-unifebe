...
export function OpenBoxPage() {
  ...
  const [status, setStatus] = useState<string>('Bipe a papeleta da caixa.')
  ...
  async function openWith(code: string) {
    ...
    try {
      ...
      vibrateOk()
      setFlash('success')
      setStatus('Caixa carregada.')
      window.setTimeout(() => setFlash('none'), 450)
    } catch (e) {
      await audio.error()
      vibrateError()
      setFlash('error')
      setStatus(e instanceof Error ? e.message : 'Falha ao abrir caixa.')
  ...
  return (
    <Screen>
      ...
      <Header title="ABRIR CAIXA" subtitle="Bipagem da papeleta (caixa/pedido)" right={<Kbd tone="info">CX</Kbd>} />
          ...
          <div className="grid gap-3">
            <Card>
              <CardBody>
                ...
                <div className="mt-4">
                  <ProgressBar value={boxView.progresso} label="Progresso da coleta" />
                </div>
                <div className="mt-3 grid grid-cols-2 gap-2 text-[12px] font-semibold text-app-muted">
                  <div className="rounded-lg border border-app-border bg-app-panel2 p-2">
                    Peças: <span className="text-app-text">{boxView.pecasTotal}</span>
                  </div>
                  <div className="rounded-lg border border-app-border bg-app-panel2 p-2">
                    Coletadas:{' '}
                    <span className="text-app-text">{boxView.pecasColetadas}</span>
                  </div>
                </div>
              </CardBody>
            </Card>
